...
from module import *
from module import BSRNN_ECA_UConv, Discriminator
...
parser.add_argument("--data_dir", type=str, default='../../dataset/VCTK-DEMAND/', help="dir of VCTK+DEMAND dataset")
...
class Trainer:
    def __init__(self, train_ds, test_ds):
        ...
        self.model = BSRNN_ECA_UConv(num_channel=64, num_layer=5, num_heads=8, upsampling_depth=4).cuda()
    ...
    def train_step(self, batch, use_disc):
        ...
        noisy_spec = torch.stft(noisy, self.n_fft, self.hop, window=torch.hann_window(self.n_fft).cuda(), onesided=True, return_complex=True)
        clean_spec = torch.stft(clean, self.n_fft, self.hop, window=torch.hann_window(self.n_fft).cuda(), onesided=True, return_complex=True)
        ...
        est_audio = torch.istft(est_spec, self.n_fft, self.hop, window=torch.hann_window(self.n_fft).cuda(), onesided=True)
