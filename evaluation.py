...
def evaluation(model_path, noisy_dir, clean_dir, save_tracks, saved_dir):
    n_fft = 512
    model = BSRNN_ECA_UConv(num_channel=64, num_layer=5).cuda()
    ...
    for audio in tqdm(audio_list):
        ...
        # 추가: 오디오 길이 확인
        if len(est_audio) == 0 or len(clean_audio) == 0:
            print(f"Skipping file {audio} due to empty or invalid audio.")
            continue
        ...
        try:
            ...
            print(f"Error computing metrics for {audio}: {e}")
            metrics = [0, 0, 0, 0, 0, 0]  # 기본값 반환
